...
func (p *balloons) validateConfig(bpoptions *BalloonsOptions) error {
	...
	if len(podResourceDevices) > 0 {
		devs := make([]string, 0, len(podResourceDevices))
		for dev := range podResourceDevices {
			devs = append(devs, dev)
		}
		sort.Strings(devs)
		log.Infof("balloon types prefer CPUs close to pod-resource devices %v; this requires the pod resources API (agent option podResourceAPI: true)", devs)
	}
...
// A name embeds the ID of the container that the hint originates from,
// so that hints can be removed from a balloon's allocator options when
// the container leaves the balloon.
...
// the pod-resource device locality hint of resourceName assigned to
// the container with the given ID.
func podResourceHintDevName(ctrID, resourceName string) string {
...
// device manager assigned to container c. The CPUs are derived from
// the NUMA nodes reported for the assigned devices via the pod
// resources API. The second return value is false if the pod resources
// are not available, if c has no such device assigned, or if the
// assigned devices carry no NUMA topology information.
...
func (p *balloons) applyPodResourcesHints(opts *cpuTreeAllocatorOptions, c cache.Container) {
	...
	opts.preferFarFromDevices = p.resolvePodResourceDevs(opts.preferFarFromDevices, opts.virtDevCpusets, c)
}
...
func (p *balloons) resolvePodResourceDevs(devs []string, virtDevCpusets map[string][]cpuset.CPUSet, c cache.Container) []string {
	...
	for _, dev := range devs {
		...
		name := podResourceHintDevName(c.GetID(), resourceName)
...
// balloon bln and its component balloons. This is done when c leaves
// the balloon so that its device locality no longer biases subsequent
// resize operations of the balloon.
func (p *balloons) removePodResourcesHints(bln *Balloon, c cache.Container) {
	if bln == nil || c == nil {
		return
	}
	prefix := podResourceHintDevPrefix + c.GetID() + "_"
	...
	for _, b := range blns {
		...
		if removed {
			...
			opts.preferFarFromDevices = filterOutPrefixDevs(opts.preferFarFromDevices, prefix)
