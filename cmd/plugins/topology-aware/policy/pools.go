...
// allowAllocatedCPUs pins container's CPUs according to what has been
// allocated for it, taking into account if the container should run
// with hyperthreads hidden.
func (p *policy) allowAllocatedCPUs(container cache.Container, allocated cpuset.CPUSet, info string) {
	allow := allocated
	hidingInfo := ""
	pod, ok := container.GetPod()
	if ok && hideHyperthreadsPreference(pod, container) {
		allow = p.sys.SingleThreadPerCore(allocated)
		if allow.Size() != allocated.Size() {
			hidingInfo = fmt.Sprintf(" (hide %s hyperthreads, allow remaining: %s)", allocated.Size()-allow.Size(), allow)
		} else {
...
func (p *policy) applyGrant(grant Grant) {
	...
	if opt.PinCPU {
		if cpuType == cpuPreserve {
			...
			if cpus.Size() > 0 {
				p.allowAllocatedCPUs(container, cpus,
...
func (p *policy) updateSharedAllocations(grant *Grant) {
	...
	for _, other := range p.allocations.grants {
		...
		if opt.PinCPU {
			...
			if exclusive.IsEmpty() {
				p.allowAllocatedCPUs(other.GetContainer(), shared,
					fmt.Sprintf("  => updating %s with shared CPUs of %s: %s...",
						other, other.GetCPUNode().Name(), shared.String()))
			} else {
				p.allowAllocatedCPUs(other.GetContainer(), exclusive.Union(shared),
