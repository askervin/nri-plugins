...
## 4. Install / reconfigure the balloons policy with PCT enabled
...
- `default` has no PCT fields. It is the implicit fallback for
  ...
  and it inherits the hardware-default CLOS (CLOS 0). Idle CPUs
  on CLOS 0 do not affect HP turbo budget as long as they stay
  in deep C-states (which they do, because `default` does not
  override `disabledCstates`).
...
spec:
  ...
  cpuClasses:
  - name: default
    # no PCT fields => CPUs fall back to the hardware-default CLOS
  - name: hp-pct
    pctPriority: high
    pctMinFreq: base
    pctMaxFreq: turbo
...
sudo intel-speed-select core-power info
...
sudo intel-speed-select core-power get-config -c 0
sudo intel-speed-select core-power get-config -c 3

# SST-TF on every punit. enable-status should be enabled (the
# plugin called TFEnable in PrepareManagedMode).
sudo intel-speed-select turbo-freq info
```
...
# in LP_CPUS.
sudo intel-speed-select -c "$HP_CPUS" core-power get-assoc | grep -E 'cpu-|clos:'
sudo intel-speed-select -c "$LP_CPUS" core-power get-assoc | grep -E 'cpu-|clos:'
```
...
In managed mode the plugin's `Shutdown()` restores SST defaults
automatically on graceful exit (CPReset → TFDisable → CPDisable
per package), so no manual `intel-speed-select` teardown is
required. Verify on the node:

```bash
# node:
sudo intel-speed-select core-power info | grep -E 'enable-status'
sudo intel-speed-select turbo-freq info | grep -E 'enable-status'
```

Both should report `disabled`. If the plugin pod was killed
ungracefully (SIGKILL, node crash) SST state may persist; reset
it with:
...
- Plugin pod log shows `Speed Select Technology (SST) support not
  ...
  the chart with `--set pct=true`. Verify with `kubectl -n
...
- Validation error `cpuClass "X": pctPriority and pctClosID are
...
- Validation error `pct: cannot mix managed (pctPriority) and
  assoc-only (pctClosID) modes`: the configuration mixes the two
