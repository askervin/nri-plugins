...
NOTE: PCT cpuClass support (`pctPriority`, `pctClosID`,
`publishExtendedResource`) is not in a released balloons chart yet, so
this guide installs balloons from an unstable build.
...
`--set pct=true` gives the plugin pod the `privileged: true`
...
- For the full balloons policy reference (other cpuClass
  ...
  integration), see [balloons.md](balloons.md).
