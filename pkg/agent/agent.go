...
type Agent struct {
	...
	nodeCfg       cfgapi.AnyConfig // node specific config resource
	groupCfgWatch watch.Interface  // group-specific/default config watch
	groupCfg      cfgapi.AnyConfig // group-specific/default config resource
	currentCfg    cfgapi.AnyConfig
...
func (a *Agent) updateNodeConfig(obj runtime.Object) {
	var cfg cfgapi.AnyConfig

	if obj != nil {
		cfg = obj.(cfgapi.AnyConfig)
	}
...
func sameConfigVersion(cfg1, cfg2 cfgapi.AnyConfig) bool {
	switch {
	...
	o1 := cfg1.GetObjectMeta()
	o2 := cfg2.GetObjectMeta()

	switch {
	case o1.UID != o2.UID:
		return false
	case o1.Generation != o2.Generation:
		return false
	case o1.Generation == 0: // config from a file
		return false
	}
