...
// for a CPU class. This is a user-friendly alternative to the
// control.cpu.classes configuration: frequencies can be specified
// with units (e.g., "3.2GHz", "800MHz"), C-states are controlled
// directly, and turbo frequency allocation is supported.
//
// Turbo allocation: When multiple CPU classes use the symbolic
// frequency "turbo" in their minFreq or maxFreq, turboPriority
// controls which class gets actual turbo frequencies. Only the
// CPU class with the highest turboPriority among those with active
// balloons gets "turbo" resolved to the real turbo frequency.
// All other classes get "turbo" resolved to the base frequency.
// When the highest-priority class no longer has active balloons,
// the next highest-priority class gets turbo. If no class sets
// turboPriority (all are 0), every class gets real turbo.
