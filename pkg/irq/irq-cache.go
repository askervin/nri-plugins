...
// setProcMount points the cache to a new procfs mountpoint and drops
// all cached data, including buffered writes.
func (c *irqCache) setProcMount(procDir string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.proc = procDir
	c.info = nil
	c.cpus = map[int]cpuset.CPUSet{}
	// Buffered writes go, too: they were calculated based on the
	// interrupts of the old mountpoint.
