...
# policy's allocatorTopologyBalancing is false, so containers should
# be packed.

# pod0's one l2load containers should find its CPUs on the same node
# with pre-reserved-avx balloons.
...
# pod1 runs in a noload balloon, should be tightly packed on the same package as pod0.
...
# pod2 runs in a noload balloon, should be tigtly packed on different package.
CPUREQ="1" MEMREQ="100M" CPULIM="1" MEMLIM=""
POD_ANNOTATION="balloon.balloons.resource-policy.nri.io: noload" CONTCOUNT=1 create balloons-busybox
report allowed
verify 'packages["pod2c0"] != packages["pod1c0"]'

# pod3c0 should be tigtly packed on same node as last pod2c0.
...
# verify freeing virtual devices by deleting and reallocating l2 and avx pods
