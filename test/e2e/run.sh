...
helm-launch() { # script API
    ...
    #         default: balloonsconfigs/default or topologyawareconfigs/default
    #
    # Example:
    ...
    vm-command "helm install --atomic -n kube-system $helm_name ./helm/$plugin \
             ...
             --set config.test.enableAPIs=true" ||
    ...
    if [ -z "$ds_name" ]; then
        case "$plugin" in
            *topology*aware*)
                ...
                [ -z "$cfgresource" ] && cfgresource=topologyawareconfigs/default
                ;;
            *balloons*)
                ds_name=nri-resource-policy-balloons
                [ -z "$cfgresource" ] && cfgresource=balloonsconfigs/default
                ;;
            *)
